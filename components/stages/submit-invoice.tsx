  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Promise.all(selectedRecords.map(async (rec) => {
        const updates = [
          { col: "50", val: timestamp }, // AX (Actual 9) Index 49
          { col: "52", val: formData.handoverBy }, // AZ (Handover By) Index 51
          { col: "54", val: subDate } // BB (Submission Date) Index 53
        ];
          ...
          <div className="mb-4 md:mb-6 p-4 md:p-6 bg-white border rounded-lg shadow-sm">
            <div className="flex items-start justify-between flex-wrap gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-900">Stage: Submit Invoice</h2>
                  <p className="text-sm text-slate-500">Track invoice submissions to accounts</p>
                </div>
              </div>
          ...
          <div className="py-4 space-y-4">
            <div className="grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg border border-gray-100 mb-6">
              <div>
                ...
                <p className="font-mono font-medium text-lg">{sheetRecords.find(r => selectedRows.has(r.id))?.data.invoiceNumber || "-"}</p>
              </div>
              <div>
                <Label className="text-xs font-semibold uppercase text-gray-500 mb-1 block">Vendor</Label>
                <p className="font-medium text-lg text-gray-900">{sheetRecords.find(r => selectedRows.has(r.id))?.data.vendorName || "-"}</p>
              </div>
            </div>
